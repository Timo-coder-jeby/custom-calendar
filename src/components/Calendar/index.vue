<template>
  <div class="calendar_wrap">
    <div class="header">
      <ul class="week">
        <li class="weekend" v-if="firstDayIsSun">日</li>
        <li>一</li>
        <li>二</li>
        <li>三</li>
        <li>四</li>
      ...
      <ul class="items">
        <li class="item" v-for="m in items" :key="m.label">
          ...
          <div class="cells">
            ...
            >
              <span>{{ day.day }}</span>
              <div class="slot" v-show="day.day && (active[0] ? day.id !== active[0].id : true) && (active[1] ? day.id !== active[1].id : true)">
                <div v-if="isToday(day) && isShowTodayText">今天</div>
                <slot
                  name="default"
                  :day="day"
                  v-if="isShowMark(day)"
...
export default {
  ...
  methods:{
    initDate () {
      // 获取月的天数
      const { beginDate,firstDayIsSun } = this
      const range = []
      ...
      const getDays = (date, index) => {
        ...
        const firstDayIsWeek = ~~dayjs(date).startOf('month').format('d') - (firstDayIsSun ? 0 : 1)
        const days = []
  ...
  computed: {
    isShowMark(){
      return day => {
        return dayjs(day.id).isAfter(dayjs())
      }
    },
    isToday() {
      return day => {
